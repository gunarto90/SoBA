...
def combine_colocation(config, p, t, d):
  ...
  if read_compressed is True:
    colocation_weekday_name = config['intermediate']['colocation_compressed'].format(p, 1, t, d)
    colocation_weekend_name = config['intermediate']['colocation_compressed'].format(p, 2, t, d)
  else:
    colocation_weekday_name = config['intermediate']['colocation'].format(p, 1, t, d)
    colocation_weekend_name = config['intermediate']['colocation'].format(p, 2, t, d)
  ...
  if compress_output is True:
    output_name = config['intermediate']['colocation_compressed'].format(p, 0, t, d)
    compression = 'bz2'
  else:
    output_name = config['intermediate']['colocation'].format(p, 0, t, d)
    compression = None
